...
PaddedPODArray<UInt32> MergeTreeReaderTextIndex::phraseSearchBlocked(const TextSearchQuery & search_query)
{
    ...
    std::vector<PaddedPODArray<UInt32>> per_token_positions(unique_tokens.size());
    {
        ...
        for (size_t u = 0; u < unique_tokens.size(); ++u)
        {
            ...
            /// Ranks of all candidates within this token's postings (ascending). When candidates
            /// are a sizable fraction of the postings — the frequent-phrase case, where this
            /// dominates warm latency — one linear walk over the materialized posting list beats
            /// per-candidate roaring rank() lookups; for sparse candidates rank() wins.
