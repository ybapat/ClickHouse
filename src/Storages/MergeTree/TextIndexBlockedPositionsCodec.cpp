...
/// Varint into a staged byte buffer: block payloads are assembled in memory because the directory
/// (their sizes) is written before them.
...
/// Parse one block payload into per-rank frequencies and the raw PFor value lane.
/// `values` holds within-document deltas; the caller reconstructs absolute positions per rank.
void parseBlock(
    ...
    PaddedPODArray<UInt32> & values)
{
    ...
    /// A PFor stream stores at least one byte per started 128-value block, so the value count a
    /// payload can legitimately declare is bounded by its size; reject before any allocation.
...
/// Append the positions of the requested local ranks from a parsed block.
void emitRanks(
...
{
    /// Flatten the sorted roaringish buckets into per-document frequencies and within-document
    /// delta positions (posting order).
    PaddedPODArray<UInt32> freqs;
    PaddedPODArray<UInt32> values;
    UInt32 current_doc = 0;
...
void TextIndexBlockedPositionsCodec::decodeAll(
    ...
    DecodeScratch & scratch)
{
    /// Sequential read from the token's start: directory, then every block in order.
    /// blob_offset 0 keeps the directory's absolute offsets relative to the token, unused here.
